...
//Android #include "waterfall.h"
//Android #include "qrunner.h"
//Android #include "macros.h"
...
//Android	if( !progdefaults.retain_freq_lock ) {
		freqlock = false;
...
//Android	}
	...
	sigsearch = 0;
/*Android	if (wf) {
//Android		bool wfrev = wf->Reverse();
//Android 		bool wfsb = wf->USB();
//Android		reverse = wfrev ^ !wfsb;
	} else
*/
		reverse = false;
	historyON = false;
...
{
	stopflag = false;
//Android	if (!wf) return;

//Android	bool wfrev = wf->Reverse();
//Android	bool wfsb = wf->USB();
//Android	reverse = wfrev ^ !wfsb;

//Android	if (progdefaults.StartAtSweetSpot) {
//Android		set_freq(progdefaults.PSKsweetspot);
//Android	} else if (progStatus.carrier != 0) {
//Android	set_freq(progStatus.carrier);
//Android	set_freq(1000.0);
#if !BENCHMARK_MODE
//Android		progStatus.carrier = 0;
#endif
//Android	} else
//Android		set_freq(wf->Carrier());
}


//Android debug	(de-inlined as it caused seg-faults in tx of RSID ???)
int	modem::get_mode()
{
	return mode;
};

int	modem::get_samplerate()
{
	return samplerate;
};
...
//Android 	REQ(put_freq, frequency);
...
{
/*Android	if (unlikely(!(cap & CAP_TX)))
		return 0;
...
{
/*Android	if (mailserver && progdefaults.PSKmailSweetSpot)
...
{
	bandwidth = bw;
//Android	put_Bandwidth((int)bandwidth);
}

void modem::set_reverse(bool on)
{
/*Android	if (likely(wf))
		reverse = on ^ (!wf->USB());
	else
		reverse = false;
*/
...
{
	set_metric(m);
//Android 	::global_display_metric(m);
}
...
{
/*Android	double sigma = sigmaN(progdefaults.s2n);
	double sn = 0;
	for (int n = 0; n < len; n++) {
...
//Android not here	pskmail_notify_s2n(s2n_ncount, s2n_avg, s2n_stddev);
}

void modem::ModulateXmtr(double *buffer, int len)
{
	txModulate(buffer, len);
/*
 * Android NOT YET
...
{
/*
 * Android Do NOTHING for the time being
...
/*
 * Android NOT to be implemented yet
