...
public:
	long long rfcarrier;
//Android 	std::string rmode;
	int carrier;
//Android	trx_mode mode;
		int mode;

//Android 	qrg_mode_t() : rfcarrier(0), rmode("NONE"), carrier(0), mode(NUM_MODES) { }
//Android 	qrg_mode_t(long long rfc_, std::string rm_, int c_, trx_mode m_)
//Android                 : rfcarrier(rfc_), rmode(rm_), carrier(c_), mode(m_) { }
	bool operator<(const qrg_mode_t& rhs) const
        {
		return rfcarrier < rhs.rfcarrier;
	}
/*Android	bool operator==(const qrg_mode_t& rhs) const
	{
...
//Android std::ostream& operator<<(std::ostream& s, const qrg_mode_t& m);
//Android std::istream& operator>>(std::istream& s, qrg_mode_t& m);

//Android #include <bitset>
//Android class mode_set_t : public std::bitset<NUM_RXTX_MODES> { };
