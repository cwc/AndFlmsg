...
//Android	scard = sc;
	...
	txphase = 0;
	videoText();
//Android not used here	strSecXmtText = progdefaults.THORsecText;
	strSecXmtText = "";
...
//Android not used here	put_sec_char(0);
	syncfilter->reset();
	datashreg = 1;
	sig = noise = 6;
...
//Android not yet	set_scope_mode(Digiscope::DOMDATA);
}
...
//Android thor::thor(trx_mode md) : hilbert(0), fft(0), filter_reset(false)
thor::thor(int md) : hilbert(0), fft(0), filter_reset(false)
{

	//Android Added access to progdefaults(preferences) in the Java side
	progdefaults.ThorCWI = getPreferenceD("THORCWI", 0.0f);
	progdefaults.THOR_FILTER  = getPreferenceB("THORFILTER", true);
	progdefaults.THOR_BW = getPreferenceD("THORBW", 2.0f);
	progdefaults.THOR_PREAMBLE  = getPreferenceB("THORPREAMBLE", true);
	progdefaults.THOR_SOFTSYMBOLS  = getPreferenceB("THORSOFTSYMBOLS", true);
	progdefaults.THOR_SOFTBITS  = getPreferenceB("THORSOFTBITS", true);

	progdefaults.ThorCWI = clamp(progdefaults.ThorCWI, 0.0f, 1.0f);
	progdefaults.THOR_BW = clamp(progdefaults.THOR_BW, 1.0f, 2.0f);
	//Android temp
	reverse = false;
...
// 8kHz modes
	...
	//Android: add slowcpu impact on filter (taken from Java modem Thor.java in AndPskmail)
	if (progdefaults.slowcpu) {
...
//Android not yet	scopedata.alloc(THORSCOPESIZE);
//Android	videodata.alloc(THORMAXFFTS * numbins );
...
void thor::recvchar(int c)
{
	if (c == -1)
		return;
//Android Not used here	if (c & 0x100)
//		put_sec_char(c & 0xFF);
	else {
		put_rx_char(c & 0xFF);
//Android Not used here		if (progdefaults.Pskmails2nreport && (mailserver || mailclient)) {
...
//Android no squelch yet	if (progStatus.sqlonoff && metric < progStatus.sldrSquelchValue) nowmag = 0;
...
//Android old complex class		avg += abs(pipe[pipeptr].vector[i]);
	...
	for (int i = 0; i < paths * numbins; i++) {
		...
		for (int j = 1; j <= numtests; j++) {
			int p = pipeptr - j;
			if (p < 0) p += twosym;
//Android old complex			cwmag = abs(pipe[j].vector[i])/numtests;
			cwmag = ((pipe[j].vector[i])/numtests).mag();
	...
	for (int i = 0; i <  paths * numbins ; i++) {
		if (cwi[i] == false) {
//Android old complex class			x = abs(pipe[pipeptr].vector[i]);
		...
		{
			//Android not yet LOG_DEBUG("cwi detected in bin %d", i);
		}
	}
...
//Android old complex class		val = abs(pipe[j].vector[prev1symbol]);
...
//Android not yet 	update_syncscope();
...
void thor::eval_s2n()
{
//Android old complex class	double s = abs(pipe[pipeptr].vector[currsymbol]);
...
//Android not used here	put_Status1(thormsg);
...
//Android not used here	snprintf(confidence, sizeof(confidence), "FEC: %3.1d%%", scalefec);
//Android	put_Status2(confidence);
}

//Android change to buffer of shorts int thor::rx_process(const double *buf, int len)
int thor::rx_process(const short *buf, int len)
{
	cmplx zref, *zp;
	cmplx zarray[1];
	int n;

//Android temp	if (slowcpu != progdefaults.slowcpu) {
//			slowcpu = progdefaults.slowcpu;
//		filter_reset = true;
//	}
...
//Android old complex class					currmag = abs(pipe_pipeptr_vector[currsymbol]);
...
//Android not yet	put_sec_char( chr );
	return chr;
}
