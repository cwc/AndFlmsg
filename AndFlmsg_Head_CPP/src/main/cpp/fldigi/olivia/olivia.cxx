...
//Android static pthread_mutex_t olivia_mutex = PTHREAD_MUTEX_INITIALIZER;
//------------------------------------------------------------------------------
...
//Android void olivia::tx_init(SoundBase *sc)
void olivia::tx_init()
{
//Android	scard = sc;
	phaseacc = 0;
...
{ // critical section
//Android	guard_lock dsp_lock(&olivia_mutex);
...
// whatever there is.
	...
	if (stopflag || (Tx->GetReadReady() < Tx->BitsPerSymbol)) {
		...
		if (stopflag)
			...
			//Android, restrict to UTF-8 byte
			c &= 0xFF;
...
//Android int olivia::rx_process(const double *buf, int len)
int olivia::rx_process(const short *buf, int len)
{
	int c;
	unsigned char ch = 0;
...
//Android	guard_lock dsp_lock(&olivia_mutex);
	...
	Rx->SyncThreshold = clamp(sldrSquelchValue / 5.0 + 3.0, 0, 90.0);
...
//Android	guard_lock dsp_lock(&olivia_mutex);
	...
	if (Tx->Preset() < 0) {
//Android not yet		LOG_ERROR("olivia: transmitter preset failed!");
		return;
	}
...
//		clamp(progStatus.sldrSquelchValue / 5.0 + 3.0, 0, 90.0) : 0.0;
	...
	if (Rx->Preset() < 0) {
//Android Not yet		LOG_ERROR("olivia: receiver preset failed!");
		return;
	}
	fragmentsize = 1024;
	set_bandwidth(Tx->Bandwidth - Tx->Bandwidth / Tx->Tones);

/*Android MUST BE FIXED IF USING PARAMETERS
...
void olivia::init()
{
	restart();
	modem::init();
//Android Not yet	set_scope_mode(Digiscope::BLANK);
}

//Android olivia::olivia(trx_mode omode)
olivia::olivia(int omode)
{
	...
	progdefaults.olivia8bit		  	= getPreferenceB("OLIVIA8BIT", true);

	progdefaults.oliviasmargin = (int) clamp(progdefaults.oliviasmargin, 2.1, 128.1);
	progdefaults.oliviasinteg  = (int) clamp(progdefaults.oliviasinteg, 2.1, 128.1);
...
/*Android		progdefaults.oliviatones = tones = 1;
